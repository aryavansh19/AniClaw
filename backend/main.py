...
def format_timestamp(ts):
    ...
    if not ts or ts == 2147483647:
        return None
    # Converts timestamp to local readable time
    dt = datetime.fromtimestamp(ts)
    return dt.strftime("%B %d at %I:%M %p")


def get_anilist_info(name: str):
    """Searches AniList by name and returns the top results"""
    # Notice we added 'coverImage { large }' to this query!
...
def get_user_by_phone(phone: str):
    """Look up user ID using the 'whatsapp_number' column in the 'users' table"""
    try:
        # Meta sends phone numbers as strings like "919876543210"
...
def fetch_single_anime(anilist_id: int):
    """Fetches full details including the next episode schedule"""
    # Added nextAiringEpisode to the query!
...
def process_anime_tracking(anilist_id: int, user_id: str):
    ...
    # 1. Check for duplicates in 'subs' table
    ...
    # 2. Get Full Details from AniList
    query = '''query ($id: Int) { Media(id: $id, type: ANIME) { 
        id title { romaji } episodes status coverImage { large }
        nextAiringEpisode { episode airingAt } 
    } }'''
    ...
    # --- 🚨 NEW SAAS LOGIC: Reject FINISHED shows ---
    ...
    # --- ⚙️ NEW SAAS LOGIC: Determine Engine State ---
    next_data = sel.get('nextAiringEpisode')

    if next_data:
        current_ep = next_data['episode'] - 1
        airing_timestamp = next_data['airingAt']
        engine_status = "waiting" # We have an exact date, the clock is ticking
    else:
        current_ep = sel.get('episodes') or 0
        airing_timestamp = 2147483647 # Placeholder for 'far future'
        engine_status = "tba" # No date yet, engine will check daily

    # 4. Save to 'anime' (Global) and 'subs' (User-specific)
    supabase.table("anime").upsert({
        ...
        "status": engine_status # Use our operational state, NOT AniList's status!
    }).execute()
    ...
    if data.get("object") == "whatsapp_business_account":
        for entry in data.get("entry", []):
            for change in entry.get("changes", []):
                ...
                # CASE A: User sent a text (Search Phase)
                if msg.get("type") == "text":
                    query_text = msg["text"]["body"]
                ...
                # CASE B: User interacted with a Menu or Button
                elif msg.get("type") == "interactive":
                    interactive_data = msg["interactive"]

                    # --- STEP 2: They tapped an item on the List Menu ---
                    if interactive_data["type"] == "list_reply":
                        ...
                        if list_id.startswith("select_"):
                            ...
                            # Fetch the image and send the confirmation card
                    ...
                    # --- STEP 3: They tapped a Button on the Image Card ---
...
def send_interactive_list(to, results):
    """STEP 1: Sends the menu of search results"""
    url = f"https://graph.facebook.com/v18.0/{PHONE_NUMBER_ID}/messages"
    headers = {"Authorization": f"Bearer {WHATSAPP_TOKEN}", "Content-Type": "application/json"}
    ...
    for anime in results[:5]:
        rows.append({
            "id": f"select_{anime['id']}",  # Prefix for list selection
...
async def track_from_web(request: Request):
    """Endpoint for the React Dashboard 'Add' button"""
    data = await request.json()
